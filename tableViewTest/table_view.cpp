...
void StyledDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
        ...
        if (view_option.state & QStyle::State_HasFocus)
        {
                ...
                painter->fillRect(view_option.rect, QColor(34, 175, 75));
        }
        //添加鼠标滑过的背景色
	...
	: QTableView(parent)
{
    ...
    styled_delegate = new StyledDelegate();
    this->setItemDelegate(styled_delegate);
...
void TableView::showToolTip(const QModelIndex &index)
{
        ...
        QString file_name = (model->index(row, colum)).data(Qt::DisplayRole).toString();
