...
//#define PYSB_BAR_SLIDE_INCREMENT_INIT 12

PYSlideButton::PYSlideButton(QWidget *parent)
    : QWidget(parent)
{
...
*/
void PYSlideButton::mousePressEvent(QMouseEvent *e)
{
...
void PYSlideButton::paintEvent(QPaintEvent *)
{
...
/*!
  painting functions
*/
void PYSlideButton::drawBar(QPainter *painter)
{
    painter->save();
...
void PYSlideButton::drawBg(QPainter *painter)
{
    painter->save();
    painter->setPen(Qt::NoPen);
...
void PYSlideButton::drawText(QPainter *painter)
{
    painter->save();
...
/*!
  private functions
*/
unsigned int PYSlideButton::GetSlideIncrement()
{
    if(m_nInc > 1)
        m_nInc --;
...
void PYSlideButton::SetSlideIncrement(unsigned int nInc)
{
    m_nInc = nInc;
}

unsigned int PYSlideButton::GetSlideIncrementInit(unsigned int nDistance)
{
    unsigned int n = 1;
...
void PYSlideButton::DoShrinking()
{
    qreal BarX = m_BarRect.topLeft().x();
    qreal BarWidth = m_BarRect.width();
...
/*!
  public interfaces
*/
void PYSlideButton::addItem(const QString &str)
{
    int ItemCnt = m_StrRectPairVec.count();
    for(int ItemIndex = 0;ItemIndex < ItemCnt ; ItemIndex++)
    {
