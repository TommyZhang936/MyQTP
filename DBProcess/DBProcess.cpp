...
CDBProcess::CDBProcess(const QString strType)
{
    ...
    {
        dbType = "";
        QMessageBox::critical(0, "ERROR", "DB type name is invalid!");
        return;
    }
    m_mapQry.clear();
    int m_siConnIdx = 0;
...
bool CDBProcess::openRecordsetBySql(const QString strSql, int idx /* = -1 */)
{
    ...
    if(false == b)
    {
        qDebug() << "Exec false -> " << pQry->lastError();
    }
    return b;
}
//关闭idx记录集
...
bool CDBProcess::getFieldsValueFromRec(int idx, const char* szFldInfo, ...) const
{
    ...
    {
        qDebug() << "QSqlQuery isValid false";
        return false;
    }
    ...
    while(szEnd != strFld)
    {
        void* pRet = va_arg(arg_ptr, void*);          //取对应返回参数
        ...
        if(strlst.size() > 1)
        {
            ch = strlst.at(1).at(0);       //取第一个字符，适应格式里面有多余字符
        }
        switch (ch.toLatin1())
        {
            case 'd':
