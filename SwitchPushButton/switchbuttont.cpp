...
SwitchButtonT::SwitchButtonT(QWidget *parent): QWidget(parent)
{
    checked = false;
    ...
    textOff = "已关";
    textOn = "已开";
    imageOff = ":/btn_check_off.png";
    imageOn = ":/btn_check_on.png";
    space = 2;
    rectRadius = 5;
    step = width() / 50;
    startX = 0;
...
SwitchButtonT::~SwitchButtonT()
{
}
void SwitchButtonT::mousePressEvent(QMouseEvent *)
{
    checked = !checked;
...
void SwitchButtonT::resizeEvent(QResizeEvent *)
{
    //每次移动的步长为宽度的 50分之一
    step = width() / 50;
    //尺寸大小改变后自动设置起点坐标为终点
...
void SwitchButtonT::paintEvent(QPaintEvent *)
{
    //绘制准备工作,启用反锯齿
...
void SwitchButtonT::drawBg(QPainter *painter)
{
    painter->save();
...
void SwitchButtonT::drawSlider(QPainter *painter)
{
    painter->save();
    painter->setPen(Qt::NoPen);
    if (!checked) {
...
void SwitchButtonT::drawText(QPainter *painter)
{
    painter->save();
    if (!checked) {
...
void SwitchButtonT::drawImage(QPainter *painter)
{
    painter->save();
    QPixmap pix;
    if (!checked) {
...
void SwitchButtonT::updateValue()
{
    if (checked) {
        if (startX < endX) {
            startX = startX + step;
...
void SwitchButtonT::setChecked(bool checked)
{
...
void SwitchButtonT::setButtonStyle(SwitchButtonT::ButtonStyle buttonStyle)
{
    this->buttonStyle = buttonStyle;
    update();
}
void SwitchButtonT::setBgColor(QColor bgColorOff, QColor bgColorOn)
{
    this->bgColorOff = bgColorOff;
    this->bgColorOn = bgColorOn;
    update();
}
void SwitchButtonT::setSliderColor(QColor sliderColorOff, QColor sliderColorOn)
{
    this->sliderColorOff = sliderColorOff;
    this->sliderColorOn = sliderColorOn;
    update();
}
void SwitchButtonT::setTextColor(QColor textColorOff, QColor textColorOn)
{
    this->textColorOff = textColorOff;
    this->textColorOn = textColorOn;
    update();
}
void SwitchButtonT::setText(QString textOff, QString textOn)
{
    this->textOff = textOff;
    this->textOn = textOn;
    update();
}
void SwitchButtonT::setImage(QString imageOff, QString imageOn)
{
    this->imageOff = imageOff;
    this->imageOn = imageOn;
    update();
}
void SwitchButtonT::setSpace(int space)
{
    this->space = space;
    update();
}
void SwitchButtonT::setRectRadius(int rectRadius)
{
